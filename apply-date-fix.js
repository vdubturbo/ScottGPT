// Apply date casting fix to Supabase database
...
async function applyFix() {
  try {
    console.log('Applying date casting fix to pgvector search function...');
    ...
    const { error: dropError } = await supabase.rpc('exec_sql', { sql_query: dropQuery });
    if (dropError && !dropError.message.includes('does not exist')) {
      console.error('Error dropping function:', dropError);
    }
    
    // Create updated function with proper casting
    const createQuery = `
    ...
    const { error: createError } = await supabase.rpc('exec_sql', { sql_query: createQuery });
    if (createError) {
    ...
    await supabase.rpc('exec_sql', { sql_query: logQuery });
    
    console.log('✅ Date casting fix applied successfully!');
    console.log('The pgvector search function now properly handles date comparisons.');
