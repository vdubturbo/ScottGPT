-- Fix pgvector search to use correct column
-- Issue: fast_similarity_search references empty embedding_vector column
-- Solution: Update function to use embedding column where vectors actually exist

-- Drop existing function
DROP FUNCTION IF EXISTS fast_similarity_search(vector(1024), FLOAT, INTEGER, TEXT[], TEXT[], TEXT, TEXT);

-- Create corrected function that uses embedding column
CREATE FUNCTION fast_similarity_search(
  query_embedding vector(1024),
  similarity_threshold FLOAT DEFAULT 0.5,
  max_results INTEGER DEFAULT 20,
  filter_skills TEXT[] DEFAULT NULL,
  filter_tags TEXT[] DEFAULT NULL,
  date_after TEXT DEFAULT NULL,
  date_before TEXT DEFAULT NULL
)
RETURNS TABLE(
  id INTEGER,
  source_id TEXT,
  title TEXT,
  content TEXT,
  content_summary TEXT,
  skills TEXT[],
  tags TEXT[],
  date_start TEXT,
  date_end TEXT,
  similarity FLOAT,
  source_type TEXT,
  source_title TEXT,
  source_org TEXT,
  source_location TEXT
) AS $$
BEGIN
  RETURN QUERY
  SELECT 
    cc.id,
    cc.source_id,
    cc.title,
    cc.content,
    cc.content_summary,
    cc.skills,
    cc.tags,
    cc.date_start,
    cc.date_end,
    -- Calculate cosine similarity using embedding column (not embedding_vector)
    (1 - (cc.embedding::vector(1024) <=> query_embedding))::FLOAT AS similarity,
    s.type AS source_type,
    s.title AS source_title,
    s.org AS source_org,
    s.location AS source_location
  FROM content_chunks cc
  LEFT JOIN sources s ON cc.source_id = s.id
  WHERE 
    cc.embedding IS NOT NULL
    AND length(cc.embedding::text) > 50  -- Basic validation that embedding exists
    AND (1 - (cc.embedding::vector(1024) <=> query_embedding)) >= similarity_threshold
    AND (filter_skills IS NULL OR cc.skills && filter_skills)
    AND (filter_tags IS NULL OR cc.tags && filter_tags)
    AND (date_after IS NULL OR cc.date_end >= date_after OR cc.date_end IS NULL)
    AND (date_before IS NULL OR cc.date_start <= date_before OR cc.date_start IS NULL)
  ORDER BY similarity DESC
  LIMIT max_results;
END;
$$ LANGUAGE plpgsql;

-- Test the corrected function
SELECT 'Testing corrected fast_similarity_search function...' as status;

-- Get vector stats to confirm vectors exist in embedding column
SELECT 
  COUNT(*) as total_chunks,
  COUNT(*) FILTER (WHERE embedding IS NOT NULL) as chunks_with_embeddings,
  COUNT(*) FILTER (WHERE embedding_vector IS NOT NULL) as chunks_with_vectors,
  ROUND(100.0 * COUNT(*) FILTER (WHERE embedding IS NOT NULL) / COUNT(*), 1) as embedding_coverage_percent
FROM content_chunks;